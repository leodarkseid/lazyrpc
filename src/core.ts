...
export class RPCBase {
  ...
  /** Custom RPC URLs to merge into the base list */
  private customRpcs: CustomRpcs = {};
  ...
  constructor(config: RPCConfig, deps: RPCDependencies) {
    ...
    this.customRpcs = config.customRpcs ?? {};
  ...
  public async getRpcAsync(type: RPCType, timeout: number = 10_000): Promise<string> {
    if (this.getValidRPCCount(type) > 0) {
  ...
  /**
   * Returns the current lifecycle status of the RPC instance.
  ...
  private init(): void {
    ...
    this.baseHttpUrls = Array.from(new Set(http)) || [];
    this.baseWsUrls = Array.from(new Set(ws)) || [];
  ...
  /**
   * Validates and merges customRpcs into the base URL lists.
   * Called within init() after the base chain list is loaded.
   *
   * Validation rules:
   * - Empty arrays throw (omit the key instead of passing an empty array)
   * - Each URL must be parseable by the URL constructor
   * - HTTP URLs must use http: or https: protocol
   * - WS URLs must use ws: or wss: protocol
   * - Duplicates with the base list are silently deduplicated
   *
   * @throws Error if any URL fails validation or an empty array is provided
   */
  private _mergeCustomRpcs(): void {
    if (!this.customRpcs || Object.keys(this.customRpcs).length === 0) return;

    const { http, ws } = this.customRpcs;

    // Validate and merge HTTP URLs
    if (http !== undefined) {
      if (!Array.isArray(http) || http.length === 0) {
        throw new Error(
          "customRpcs.http must be a non-empty array of URL strings. " +
          "Omit the field entirely if you have no custom HTTP endpoints.",
        );
      }
      for (const url of http) {
        this._validateCustomUrl(url, "http");
      }
      this.baseHttpUrls = Array.from(new Set([...this.baseHttpUrls, ...http]));
    }

    // Validate and merge WS URLs
    if (ws !== undefined) {
      if (!Array.isArray(ws) || ws.length === 0) {
        throw new Error(
          "customRpcs.ws must be a non-empty array of URL strings. " +
          "Omit the field entirely if you have no custom WebSocket endpoints.",
        );
      }
      for (const url of ws) {
        this._validateCustomUrl(url, "ws");
      }
      this.baseWsUrls = Array.from(new Set([...this.baseWsUrls, ...ws]));
    }
  }

  /**
   * Validates a single custom RPC URL for correct format and protocol.
   * @param url - The URL string to validate
   * @param type - Expected type: "http" for HTTP(S) URLs, "ws" for WebSocket URLs
   * @throws Error with a specific message identifying the bad URL
   */
  private _validateCustomUrl(url: string, type: "http" | "ws"): void {
    let parsed: URL;
    try {
      parsed = new URL(url);
    } catch {
      throw new Error(
        `Invalid URL in customRpcs.${type}: "${url}" is not a valid URL`,
      );
    }

    if (type === "http" && !["https:", "http:"].includes(parsed.protocol)) {
      throw new Error(
        `Invalid protocol in customRpcs.http: "${url}" uses "${parsed.protocol}" — ` +
    ...
    if (type === "ws" && !["wss:", "ws:"].includes(parsed.protocol)) {
      throw new Error(
        `Invalid protocol in customRpcs.ws: "${url}" uses "${parsed.protocol}" — ` +
        `expected "ws:" or "wss:"`,
      );
    }
  }
        ...
        for (let i = 0; i < allUrls.length; i += BATCH_SIZE) {
          ...
          for (const result of batchResults) {
            if (result.status === "fulfilled") {
              instance._resolveGetAsyncQueue(result.value)
  ...
  /**
   * Validates a JSON-RPC 2.0 response from an `eth_blockNumber` call.
   * Ensures the response has the correct shape, matching ID, no error,
   * and a hex-encoded result.
   *
   * @param data - The parsed JSON-RPC response object.
   * @param expectedId - The request ID that should be echoed back.
  ...
  private validateJsonRpcResponse(data: any, expectedId: number): void {
    ...
    assert(
      !data.error,
      `JSON-RPC error: ${data.error?.message ?? JSON.stringify(data.error)}`,
    );
    assert(
